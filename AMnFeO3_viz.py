...
for i in files:
	if (re.search('A_tempe', i)):
		...
		complete.append([i, time.ctime(int(file_time))])
...
file_prefix = complete[int(myfile)][0][:-24]
...
file_time = file_prefix.split('x=')[0].split('_')[-2]
print(time.ctime(int(file_time)))

print("x=",x,",L=",L)

if raw_input("use non-default parameters for burn and fit temperatures?"):
        steps_to_burn = int(raw_input("steps to burn?"))
        A_fit_temperature_min = float(raw_input("A fit temperature min?"))
        A_fit_temperature_max = float(raw_input("A fit temperature max?"))
        G_fit_temperature_min = float(raw_input("G fit temperature min?"))
        G_fit_temperature_max = float(raw_input("G fit temperature max?"))
else:
...
a_xfit = spop.optimize.fmin(op_fit, \
					...
					x0=(-2, A_fit_temperature_max*1.01, 0.333), args = (A_temperature_sweep_array_slice, A_tot_mean_temperature_array_slice), disp=1)

					
#fit the a-type order parameter here
a_arbitrary_scale_factor = a_xfit[0]
a_T_N = a_xfit[1]
a_beta = a_xfit[2]
A_temperature_sweep_array_slice_manypoints = np.linspace(A_fit_temperature_max, A_fit_temperature_max, 100)

#fit the g-type order parameter here
xfit = spop.optimize.fmin(op_fit, \
					maxfun=5000, maxiter=5000, ftol=1e-6, xtol=1e-5,\
					x0=(-2.5, G_fit_temperature_max*1.01, 0.333), args = (G_temperature_sweep_array_slice, G_tot_mean_temperature_array_slice), disp=1)

g_arbitrary_scale_factor = xfit[0]
g_T_N = xfit[1]
g_beta = xfit[2]
g_temperature_sweep_array_slice_manypoints = np.linspace(G_fit_temperature_max, G_fit_temperature_max, 100)
